...
- **公开搜索（Bing，无需密钥）**：默认选项。通过公开 RSS 搜索获取标题、链接和摘要，适配能调用函数工具的模型。先过滤明显跑题的结果和错误页，再取最多 5 条来源；异常或没有相关结果时，对相同查询直连 Bing 重试一次。两次都失败则记录检索失败，不缓存无关结果。直连补救只访问固定的 Bing 搜索地址，不影响模型 API 或其他网页的代理设置。模型可用 `read_page` 进一步核对正文。
- **内置搜索**：使用所选模型同一平台的 `/v1/responses`，发送 `web_search`。只记录实际搜索事件、来源及引用。搜索内容在供应商侧交给本次模型，接口可能仅向客户端返回来源 URL；页面上没有摘要并不代表本次模型没有搜索内容。后续引用带回的标题会合并到相同 URL，不被去重丢弃。使用这一模式会为该次答题选择 Responses 协议，自定义模型脚本不参与该模式。
...
Tavily、Bing、SearXNG 出现额度耗尽、限流、HTTP 错误、网络异常、单次搜索超时、解析错误、缺少配置或无可用结果时，本题自动切换到当前模型同一平台的 Responses 内置搜索。Bing 会先执行既有的一次直连补救。切换不修改保存的供应商设置；下一题仍先使用用户选择的供应商。

自动与每题检索模式均生效；自动模式只有实际尝试搜索失败才切换。同题后续模型直接沿用兜底路径，不再请求已失败的通用搜索服务。重启模型调用时保留原题、图片、模型配置和整题截止时间，移除未完成的工具调用历史；使用模型平台密钥，不向模型接口发送 Tavily 密钥。原生兜底须取得真实搜索事件和来源，平台不支持或再次失败时进入待复核，不循环切换。

失败的通用搜索尝试仍显示在检索次数中，但不占用有效搜索预算，确保上限设为 1 时仍有一次内置搜索机会；成功搜索和内置搜索事件正常计入上限。用户取消、整题超时、工具参数无效和预算耗尽不触发新的搜索请求。网页正文读取失败仍由模型选择其他来源，不视为搜索供应商故障。内置搜索使用当前模型平台的计费规则。

2026-09-18 CodeQ 流式端到端验证：本地模拟 Tavily HTTP 432 额度耗尽，再使用真实模型平台调用兜底。`gpt-5.6-luna` 在一次 Responses 请求中返回真实搜索事件和 15 条来源，整个测试约 9.6 秒；`grok-4.6` 同路径未返回搜索事件和来源，程序按预期进入待复核。此结果只代表当次渠道表现，未自动改用其他模型。

## 时间与费用控制

检索按事实核验组织：先保留题干中的实体、时间、范围、否定与程度条件，搜索关键概念或关系；公开资料足以支撑判断时即可作答，不要求找到题库原题或标准答案。遇到只显示“查看答案”的页面，应换公开事实来源，已有相关摘要时优先核对正文，避免同义重复搜索。

通用工具每轮会更新剩余预算，搜索次数用完后移除 `web_search`，但仍保留预算内可用的 `read_page`；阅读次数用完则移除阅读工具。工具结果携带当前剩余次数。网页读取遇到访问限制、非文本内容或请求失败时返回明确的未取得正文提示，同题重复读取该 URL 不再请求网络或消耗页数；原有搜索摘要不会被登录墙文本覆盖。访问限制识别是启发式提示，不代表对网页全部内容的语义判定。内置搜索在供应商侧执行，其内部工具动作通过提示与本地事件预算约束。

2026-09-19 对用户反馈的“1939 年德国入侵波兰与世界反法西斯战争起点”判断题进行单题 CodeQ Grok + Tavily 实测：2 次搜索、3 次正文读取，来源包括新华网和人民网，最终返回结构化答案；工具列表按剩余预算从搜索切换至阅读，再关闭。此为工作流定向验证，不作整体答题准确率或耗时改善的证明。
...
内置搜索来源列表与答案实际引用分开标识；未回传摘要的条目标注“仅来源链接”。其他基础模型或裁决模型仅复用客户端真实收到的摘要或正文，不把前序请求的裸链接当作已经阅读的证据。每题检索模式下，后续模型在剩余预算内自行搜索；预算已耗尽且没有可共享文本时进入待复核。多模型复核更适合能回传摘要的 Tavily 等通用搜索，或按实际需求提高每题检索上限。程序不会为了补齐界面自动抓取全部来源页面。

Bing 的相关性过滤是关键词粗筛，使用英文词和中文相邻双字，排除明显无关的结果；它不是语义核验，可能遗漏只使用同义表达的资料。搜索通道标题会原样回显查询，因此不作为相关性依据。模型收到摘要时显示“已接收检索资料”，实际正文阅读单独计数。2026-09-18 的定向排查发现同一中文查询在不同请求头和网络路径下会收到完全不同主题的 RSS 条目；仅切换中文入口或检查 HTTP 200 不足以确认搜索成功。
